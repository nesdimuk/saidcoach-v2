...
const DOWN_THRESHOLD = 125;
const UP_THRESHOLD = 170;
...
function resetCounter() {
  ...
  updateStatus("Reiniciado. Colócate de lado frente a la cámara.");
}
...
function evaluateSide(landmarks, indices, name) {
...
function selectSideLandmarks(landmarks) {
  const right = evaluateSide(
    landmarks,
...
function onResults(results) {
  ...
  if (results.poseLandmarks) {
    ...
    const selectedSide = selectSideLandmarks(landmarks);
    ...
    if (!selectedSide) {
      updateStatus("No detecto bien tu pierna. Ponte de lado frente a la cámara.");
      wasBelowThreshold = false;
      readyForCounting = false;
      consecutiveReadyFrames = 0;
      currentSide = null;
    ...
    const { hip, knee, ankle, name: sideLabel } = selectedSide;
    ...
    if (currentSide !== sideLabel) {
      ...
      wasBelowThreshold = false;
      consecutiveReadyFrames = 0;
      updateStatus(`Detecto tu pierna ${sideLabel}. Ponte de pie para comenzar.`);
    }
    ...
    if (!readyForCounting) {
      if (angle >= UP_THRESHOLD) {
        ...
        if (consecutiveReadyFrames >= READY_FRAMES_REQUIRED) {
          ...
          updateStatus(`Pierna ${sideLabel} lista. Baja para comenzar.`, true);
        }
      } else {
        consecutiveReadyFrames = 0;
        updateStatus(`Ponte erguido mostrando tu pierna ${sideLabel} a la cámara.`);
      }
      canvasCtx.restore();
      return;
    }
...
function initialize() {
  ...
  camera.start().then(
    () => {
      updateStatus("Ponte de lado y muestra una pierna completa a la cámara.");
    },
    (err) => {
